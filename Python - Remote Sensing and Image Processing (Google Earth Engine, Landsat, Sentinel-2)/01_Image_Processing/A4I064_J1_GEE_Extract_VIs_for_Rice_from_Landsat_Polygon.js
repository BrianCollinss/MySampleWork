// This script extracts the band values from Landsat and calculates the 14 vegetation indices for the field polygons for rice using Google Earth Engine. It reads "ricepolygon" as the assets in which "ricepolygon" is uploaded as the field polygon shape file. It creates two csv files as the output in the Google Drive which has been linked to GEE:1) "VIs_Landsat_Rice_Polygons_MinMaxMean.csv" which includes the minimum, maximum and average values of the VIs within the polygon and 2) "VIs_Landsat_Rice_Polygons.csv" which includes the average values of VIs within the polygon. The second file is the one that is used for further analysis. Both the files also report the centroid latitudes and longitudes.
...
// --- EXPORT CSVs ---
Export.table.toDrive({collection: meanOnlyClean, description:'VIs_Landsat_Rice_Polygons', fileFormat:'CSV', folder: 'GEE_VIs'});
Export.table.toDrive({collection: MinMaxMeanClean, description:'VIs_Landsat_Rice_Polygons_MinMaxMean', fileFormat:'CSV', folder: 'GEE_VIs'});
