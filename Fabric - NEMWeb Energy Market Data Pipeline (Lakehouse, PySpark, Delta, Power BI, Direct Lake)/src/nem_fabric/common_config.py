...
class Settings(BaseModel):
    ...
    nemweb_base_url: HttpUrl = cast(HttpUrl, "https://nemweb.com.au/Reports/Current/")
    nemweb_dispatchis_url: HttpUrl = cast(
        HttpUrl,
        "https://nemweb.com.au/Reports/Current/DispatchIS_Reports/",
    )
    nemweb_public_prices_url: HttpUrl = cast(
        HttpUrl,
...
def load_yaml(path: str | Path) -> dict[str, Any]:
    ...
    can call this function from different working directories. Fabric notebooks
    may load checked-in config from Lakehouse Files when only the package folder
    is on `sys.path`.
    """
    ...
    if not yaml_path.is_absolute():
        ...
        if not yaml_path.exists():
            fabric_path = FABRIC_FILES_ROOT / Path(path)
            if fabric_path.exists():
                yaml_path = fabric_path
...
def load_settings(env_path: str | Path | None = None) -> Settings:
    ...
    # Pydantic validates URL shapes and numeric bounds before callers use them.
