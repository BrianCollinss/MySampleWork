//// This script extracts the band values from Sentinel-2 and calculates the 14 vegetation indices for the field polygons for mango using Google Earth Engine. It reads "mangopolygon" as the assets in which "mangopolygon" is uploaded as the field polygon shape file. It creates two csv files as the output in the Google Drive which has been linked to GEE:1) "VIs_Sentinel2_Mango_Polygons_MinMaxMean.csv" which includes the minimum, maximum and average values of the VIs within the polygon and 2) "VIs_Sentinel2_Mango_Polygons.csv" which includes the average values of VIs within the polygon. The second file is the one that is used for further analysis. Both the files also report the centroid latitudes and longitudes.

var crop_region = ee.FeatureCollection("users/brcollins2020/MangoPolygon");
...
// --- EXPORT CSVs --- //
Export.table.toDrive({collection: meanOnlyClean, description: 'VIs_Sentinel2_Mango_Polygons', fileFormat: 'CSV', folder: 'GEE_VIs'});
Export.table.toDrive({collection: MinMaxMeanClean, description: 'VIs_Sentinel2_Mango_Polygons_MinMaxMean', fileFormat: 'CSV', folder: 'GEE_VIs'});