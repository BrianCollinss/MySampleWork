...
Purpose:
Provides a controlled promotion step in the MLOps release process, allowing
a validated deployment such as production to take over live traffic from
another deployment such as staging.
...
def main(args):
    ...
    if args.to_deployment not in current_traffic:
        print(
            f"Warning: '{args.to_deployment}' is not currently in the "
            "endpoint traffic map. Proceeding anyway."
        )

    # Warn if the source deployment is not currently receiving traffic
    if args.from_deployment not in current_traffic:
        print(
            f"Warning: '{args.from_deployment}' is not currently in the "
            "endpoint traffic map."
        )
    ...
    print(
        f"Promoted deployment '{args.to_deployment}' on endpoint '{args.endpoint_name}'"
    )
