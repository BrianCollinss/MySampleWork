...
Source URL: https://apsimnextgeneration.netlify.app/usage/commandline/batch
Source domain: apsimnextgeneration.netlify.app
Fetched at: 2026-04-19T09:51:23.976286+00:00
...
the
--apply
switch can be used in conjunction with the
--batch
switch.
...
a placeholder is a name that starts with a
$
symbol. An example would be
$weather-file-name
.
...
run
A batch file, this is a csv file with headers that match the placeholders (minus the
$
symbols)
...
19479.met
Sim0003
To run this we would run something like:
"C:\Program Files\APSIM<your version number>\bin\Models.exe" --apply config-file-name.txt --batch batch-file-name.csv
