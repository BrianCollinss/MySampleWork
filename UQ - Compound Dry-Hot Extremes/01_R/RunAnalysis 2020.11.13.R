...
if (DoGraphs) {
  ...
  #=========================================================================================================================
  {
    ...
    p <- Plot_Grid(list(p1,p2),CommLegend=T,LegPos=T,RemTitles=F,RemLabs=F,RemAxText=F,
  ...
  #=========================================================================================================================
  {
    for (V in Vars) {
      ...
      for (S in 0:5) {
        ...
        p <- DoGGPlot(Data=Temp,Type='linep',xCol='Year',yCol=V,
                      hFacet='State2',NRow=4,CColors='black',RemGrids=T,PSize=1.0,PShape=18,
                      xLab='Year',yLab='Var (%)',LegPos='top',FacetScale='free_y',
                      LegKeyText=LegKeyText,AxTxSize=AxTxSize,Alpha=1,LegScale=5,yAngle=0,xBreaks=seq(2019,1889,-20),
                      ShowReg=F,RegDegree=1,RegType=3,AddRegEq='Eq-R2-Ast-Mean',RegEqPos='topleft',
                      RegEqSize=2,RegEqMarg=10,RegEqHVadj=c(0,5),RegXfrom1=T,RegEqBold=T,RegEqAddNames=F,RegEqDigs=c(3,2),
                      HLine=mean,HLineCol='red',HLineType=3)
        Plots[[length(Plots)+1]] <- p
      }
  ...
  #=========================================================================================================================
  {
    for (i in 1:2) {
      ...
      Plots1 <- list()
      for (S in 0:5) {
        Temp <- copy(RPTrendsState[Seas==S & TrendVar%in%Cols & Period%in%last(Periods,2)])
        Temp[, IsTrend:=Trends[IsTrend+2]]
        Temp[, IsTrend:=factor(IsTrend,levels=Trends)]
        ...
        if (i==1) ExpPath <- paste0(Dir$Analyses,sprintf("/1 - Bar - State Trend of RP - %s.pdf",Seasons[S+1]))
        if (i==2) ExpPath <- paste0(Dir$Analyses,sprintf("/1 - Bar - State Trend of Vars - %s.pdf",Seasons[S+1]))
        ...
        p1 <- DoGGPlot(Data=copy(Temp),Type='bar',xCol='TrendVar',yCol='LN_SL',fCol='IsTrend',
                       hFacet='State2',vFacet='Period',FacetScale='free_y',FColors=TrCols,CColors='black',
                       LegPos='top',LegKeyText=LegKeyText,UseMean=T,ErrBarType='se',BarLabSize=1.8,BarLabDigs=2,
  ...
  #=========================================================================================================================
  {
    for (S in 0:5) {
      ...
      Temp <- copy(RPTrends[IsGrid==T & Seas==S & Period%in%last(Periods,2) & TrendVar%in%Cols])
      Temp <- merge(Temp,Stations[,-c('IsGrid','State2')],by=c('Name','ID'),sort=F)
      Temp[, TrendVar:=Change_Levels(TrendVar,OldLevs=c('RPt','RPr','RP'),NewLevs=c('Hot Years','Dry Years','Hot and Dry Years'),AsFactor=T)]
      ...
      p <- DoGGPlot(Data=Temp[Period%in%last(Periods,2)],Map=AusMap,Type='point-pval',xCol='Long',yCol='Lat',cCol=c('LN_SL','YP_P'),
  ...
  #=========================================================================================================================
  {
    for (S in 0:5) {
      ...
      for (i in seq_along(Vars)) {
        ...
        Plots[[i]] <- DoGGPlot(Data=Temp[Period%in%last(Periods,2)],Map=AusMap,Type='point-pval',xCol='Long',yCol='Lat',cCol=c('LN_SL','YP_P'),
  ...
  #=========================================================================================================================
  {
    for (S in 0:5) {
      for (i in 1:2) {
        ...
        p1 <- DoGGPlot(Data=Temp[Period%in%last(Periods,2)],Map=AusMap,Type='points',xCol='Long',yCol='Lat',cCol='IsTrend',
